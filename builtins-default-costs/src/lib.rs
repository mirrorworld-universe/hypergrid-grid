...
///
#[allow(dead_code)]
const TOTAL_COUNT_BUILTS: usize = 14;
#[cfg(test)]
...
pub const NON_MIGRATING_BUILTINS_COSTS: &[(Pubkey, BuiltinCost)] = &[
    ...
    // Sonic: XXX: double check me later. Previously there was a comment:
    // DO NOT ADD MORE ENTRIES TO THIS MAP
    (
        sonic_account_migrater_program::id(),
        BuiltinCost::NotMigrating(NotMigratingBuiltinCost {
            native_cost: sonic_account_migrater_program::processor::DEFAULT_COMPUTE_UNITS,
        }),
    ),
    (
        sonic_fee_settlement_program::id(),
        BuiltinCost::NotMigrating(NotMigratingBuiltinCost {
            native_cost: sonic_fee_settlement_program::processor::DEFAULT_COMPUTE_UNITS,
        }),
    ),
];
