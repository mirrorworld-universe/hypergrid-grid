...
declare_process_instruction!(Entrypoint, DEFAULT_COMPUTE_UNITS, |invoke_context| {
    ...
    match limited_deserialize(instruction_data)? {
        ...
        ProgramInstruction::MigrateSourceAccounts { node_id, addresses } => {
            Processor::migrate_source_accounts(invoke_context, node_id, addresses)
        }
        ProgramInstruction::InitializeDataAccount => {
            Processor::initialize_data_account(invoke_context)
        }
    }
});
...
impl Processor {
    fn initialize_data_account(invoke_context: &mut InvokeContext) -> Result<(), InstructionError> {
        ...
        for i in 0..n {
                ...
                && account.is_writable()
            {
                ic_msg!(
                    invoke_context,
                    "Account {:?} is not signer and writable.",
                    account.get_key()
                );
        ...
        ic_msg!(
            invoke_context,
            "Data Account is initialized at slot {}.",
            clock.slot
        );
    ...
    fn migrate_remote_accounts(
        ...
        for i in 0..n {
                ...
                && account.is_writable()
            {
                ic_msg!(
                    invoke_context,
                    "Account {:?} is not signer or writable.",
                    account.get_key()
                );
        ...
        ic_msg!(
            invoke_context,
            "{} Remote Accounts are migrated at slot {}.",
            addresses.len(),
            clock.slot
        );
    ...
    fn migrate_source_accounts(
        ...
        for address in addresses.iter() {
            ic_msg!(
                invoke_context,
                "Account {:?} is migrated at slot {:?} from {:?}.",
                address,
                slot,
                node_id
            );
        ...
        ic_msg!(
            invoke_context,
            "{} Remote Accounts are migrated from {} at slot {}.",
            addresses.len(),
            node_id,
            clock.slot
        );
    ...
    fn deactivate_remote_accounts(
        ...
        for address in addresses.iter() {
            ic_msg!(
                invoke_context,
                "Account {:?} is deactivated in cache.",
                address
            );
        ...
        ic_msg!(
            invoke_context,
            "{} Remote Accounts are already deactivated at slot {}.",
            addresses.len(),
            clock.slot
        );
