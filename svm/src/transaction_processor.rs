...
// Sonic:
use {log::*, solana_bincode::limited_deserialize};
...
impl<FG: ForkGraph, A: AccountsDb> Default for TransactionBatchProcessor<FG, A> {
    fn default() -> Self {
        Self {
            ...
            genesis_accounts_pubkeys: Arc::new(HashSet::new()),
        }
    }
}
...
impl<FG: ForkGraph, A: AccountsDb> TransactionBatchProcessor<FG, A> {
    ...
    pub fn new(
        ...
        let processor = Self::new_uninitialized(slot, epoch, accounts_db, genesis_accounts_pubkeys);
        {
    ...
    fn check_remote_accounts(&self, tx: &impl SVMTransaction) -> bool {
        ...
        let instruction = match limited_deserialize(ix.data, solana_packet::PACKET_DATA_SIZE as u64)
        {
            Err(_) => {
    ...
    fn migrate_remote_accounts(&self, tx: &impl SVMTransaction) {
        ...
        msg.instructions_iter()
            ...
            .filter_map(|ix| {
                match limited_deserialize(ix.data, solana_packet::PACKET_DATA_SIZE as u64) {
                    Ok(ok) => Some(ok),
                    Err(err) => {
...
mod tests {
    ...
    fn test_replenish_program_cache_with_nonexistent_accounts() {
        ...
        let batch_processor = TransactionBatchProcessorTest::new(
            0,
            0,
    ...
    fn test_validate_transaction_fee_payer_exact_balance(
        ...
        let result =
            TransactionBatchProcessorTest::<TestForkGraph>::validate_transaction_nonce_and_fee_payer(
                &mut account_loader,
                &message,
    ...
    fn test_validate_transaction_fee_payer_is_nonce(formalize_loaded_transaction_data_size: bool) {
        ...
        // Sufficient Fees
        {
            ...
            let result = TransactionBatchProcessorTest::<TestForkGraph>::validate_transaction_nonce_and_fee_payer(
                   &mut account_loader,
                   &message,
                   tx_details,
